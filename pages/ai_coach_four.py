...
class AICoach:
    ...
    def __init__(self):
        ...
        # 關節權重（反拍挑球以手腕/前臂旋轉為主，肩膀與手肘伸展重要性低）
        self.joint_weights = {
            "16": 3.5,  # wrist（反拍挑球核心，前臂旋轉+手腕托球）
            "14": 1.5,  # elbow（彎曲貼身為主，非伸展發力，權重降低）
            "12": 0.8,  # shoulder（反拍挑球幾乎不需轉肩，權重最低）
            "11": 1.2,
            "23": 1.0,
            "24": 1.0
        }

        # threshold（挑球動作幅度小，誤差數值通常較小，判定收緊）
        ...
        self.relaxed_factor = {
            "prep": 1.1,
            "impact": 1.1,
            "follow": 1.6   # 挑球收拍動作幅度小，放寬係數略降於高遠球版本
        }
    ...
    def _joint_message(self, phase, joint, level):
        ...
        if phase == "prep":
            if joint == "16":  # wrist
                if level == "bad":
                    return "準備時手腕過早翻轉出力，建議保持放鬆，拍面朝上等來球接近再動作"
                else:
                    return "準備時手腕稍微提前動作，可再放鬆延後一點"
            elif joint == "14":  # elbow
                if level == "bad":
                    return "準備時手肘過度伸展或離身過遠，建議彎曲收於身體前側，拍面自然朝上迎球"
                else:
                    return "準備手肘位置略嫌外展，收近身體一點會更好掌握"
            elif joint == "12":  # shoulder
                if level == "bad":
                    return "準備時肩膀轉動過大，反拍挑球不需要轉體，建議以手肘手腕小動作迎球為主"
                else:
                    return "準備肩部轉動稍多，動作可以更集中在手臂"

        # ---------- impact ----------
        elif phase == "impact":
            if joint == "16":  # wrist
                if level == "bad":
                    return "擊球瞬間前臂旋轉與手腕托球動作不足，建議觸球時做出向上旋轉的托送動作"
                else:
                    return "擊球時手腕托送力道稍弱，可再多一點旋轉托球的感覺"
            elif joint == "14":  # elbow
                if level == "bad":
                    return "擊球時手肘過度伸展出力，反拍挑球應保持手肘微彎，靠前臂旋轉托球而非手肘用力"
                else:
                    return "擊球時手肘出力稍多，嘗試保持微彎讓前臂主導"
            elif joint == "12":  # shoulder
                if level == "bad":
                    return "擊球時肩膀過度參與發力，反拍挑球應以前臂與手腕為主，肩膀盡量保持穩定"
                else:
                    return "擊球時肩部稍微用力過多，讓前臂旋轉來完成托球即可"

        # ---------- follow ----------
        else:
            if joint == "16":  # wrist
                if level == "bad":
                    return "收拍時手腕動作過大，反拍挑球後應迅速回穩準備下一拍，不需要大幅延伸"
                else:
                    return "收拍手腕稍微多餘動作，試著擊球後就自然停下"
            elif joint == "14":  # elbow
                if level == "bad":
                    return "收拍時手肘延伸過多，建議擊球後立即回收貼身，保持準備下一拍的姿勢"
                else:
                    return "收拍手肘延伸稍多，可以再快一點收回準備位置"
            elif joint == "12":  # shoulder
                if level == "bad":
                    return "收拍時肩膀持續轉動過多，反拍挑球後應盡快回正準備下一次來球"
                else:
    ...
    def _phase_good_msg(self, phase):
        if phase == "prep":
            return "準備動作簡潔到位，手肘貼身、拍面迎球角度良好"
        elif phase == "impact":
            return "擊球時前臂旋轉與手腕托球控制精準，出球穩定"
        else:
