...
class AICoach:
    ...
    def _joint_message(self, phase, joint, level):
        ...
        if phase == "prep":
            if joint == "16":  # wrist
                if level == "bad":
                    return "引拍時手腕過早扣腕，建議保持放鬆延後至擊球前才發力"
                else:
                    return "引拍時手腕稍微提前出力，可再晚一點收緊"
            elif joint == "14":  # elbow
                if level == "bad":
                    return "引拍時手肘偏低且夾緊，建議抬至與肩同高並保持外開蓄力"
                else:
                    return "引拍手肘高度略低，稍微抬高可增加蓄力空間"
            elif joint == "12":  # shoulder
                if level == "bad":
                    return "引拍時肩膀轉動不足，建議加大轉體幅度帶動手臂蓄力"
                else:
                    return "引拍肩部轉動略嫌不足，轉體再大一點效果更好"
        ...
        # ---------- follow ----------
            ...
            if joint == "16":  # wrist
                if level == "bad":
                    return "收拍時手腕未自然釋放向上翻轉，建議讓手腕順慣性甩出"
                else:
